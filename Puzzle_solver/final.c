...
int main( int argc, char *argv[]) {
    ...
    if (p!=0 && p!=(P/2)-1 && p!=(P/2) && p!= P-1){
        if (q==0) {
            MPI_Recv(&Maux, 1, mpi_element, p-1, tag, MPI_COMM_WORLD, &status);
        }
        else if (q==2) {
            MPI_Recv(&Maux, 1, mpi_element, p-1, tag, MPI_COMM_WORLD, &status);
         }
                ...
                for (j=0; j<N; j++){
                    ...
                    if (q==0) {
                        ...
                        if (p!=(P/4)-1 && k==0 && l==(p+1)*h-1 && M[k][l].ord == 1 ){
                            MPI_Send(&M[k][l], 1, mpi_element, p+1, p+1, MPI_COMM_WORLD);
                        }
                    }
                        ...
                        if (p!=(P/4) && k==0 && l==(p+1)*h-1 && M[k][m].ord == 1 ){
                          MPI_Send(&M[k][m], 1, mpi_element, p-1, p-1, MPI_COMM_WORLD);
                        }
                    }
                    else if (q==2) {
                        n=N-k-1;
                        ...
                        if (l==p*h && k!=0) {
                            ...
                            && (M[i][j].s_c==M[n+1][m].n_c)&& (M [i][j].e_s == 0    ) ) {
                                swapel(&M[i][j], &M[n][m]);
				
                            }
                        }
                        if (p!=(3/4)*P-1 && k==0 && l==(p+1)*h-1 && M[n][m].ord == 1 ){
                          MPI_Send(&M[n][m], 1, mpi_element, p+1, p+1, MPI_COMM_WORLD);
                        }
                    }
                    else if (q==3) {
                        n=N-k-1;
